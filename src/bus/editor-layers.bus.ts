...
import { addLayerAndActivate, removeLayer, removeTile, setActiveLayer, silentSetActiveLayer, silentSetLayers, silentUpdateLayers, updateLayer, updateTile } from "@store/slices/layers";
...
export class EditorLayersBus extends BaseBusLayers {
	...
	handleActiveLayerChange({ id }: { id: string }, meta?: MetaData) {
		const dispatch = getDispatch(this.store)
	...
	handleLayersCreation(layers: ILayerModel[], meta?: MetaData) {
		...
		if (meta?.reason === 'hydratation') {
			...
			return dispatch(silentSetLayers({
				data: mappedLayers
			}))
		}
	}
