...
export const registerTool = createDocumentScopedThunk<
  IToolModel,
  IToolModel
>(
  'tools/registerTool',
...
>(
  ...
  ({ documentId, name, config }) => {
    DocumentsApi.withDocument(documentId).updateToolConfig(name, config || {});
    return { name, config }
  }
);

export const deactivateTool = createDocumentScopedThunk<
  void,
  void
>(
...
export const deactivateAllTools = createDocumentScopedThunk<
  void,
  void
>(
