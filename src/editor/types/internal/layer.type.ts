...
export interface ILayer extends IEventEmitter<LayerEventMap>, ILayerModel {
  ...
  setRegionToTile(startX: number, startY: number, inputString: string, tile: { x: number; y: number }): void;
  clearRegion(startX: number, startY: number, width: number, height: number): void;
  fillRegionToTile(startX: number, startY: number, width: number, height: number, char: string, tile: { x: number; y: number }): void;
