...
export class CameraControlTool extends BaseTool implements ITool {
  ...
  constructor(coreApi: CoreApi) {
    ...
    this.toolsBus.withTool(this.name).on('zoom-change::request', (data?: { percentage: number }) => {
  ...
  private handleMouseDown(event: MouseEvent): void {
    if (event.button === 1 || event.button === 2) {
      const { x: mouseX, y: mouseY } = this.camera.getMousePosition({ x: event.clientX, y: event.clientY });
