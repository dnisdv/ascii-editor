...
const createMouseEvent = (type: string, clientX: number, clientY: number, buttons: number = 1): MouseEvent => {
...
describe('SelectTool', () => {
  ...
  const performSelectionDrag = (startCellX: number, startCellY: number, endCellX: number, endCellY: number) => {
    const startCoords = cellToWorld(startCellX, startCellY);
    const endCoords = cellToWorld(endCellX, endCellY);
...
AAA`;
  ...
  describe("When content is already selected", () => {
    ...
    beforeEach(() => {
      ...
      performSelectionDrag(initialCellStartX, initialCellStartY, DRAWING_WIDTH_CELLS, DRAWING_HEIGHT_CELLS);
      expect(modeContext.getCurrentModeName()).toBe(SelectionModeName.SELECTED);
    });
    ...
    it('should transition SELECTED -> ROTATING -> SELECTED on clicking a rotation handle and dragging', () => {
      ...
      modeContext.onMouseMove(createMouseEvent('mousemove', rotationHandleClientX, rotationHandleClientY));

      modeContext.onMouseDown(createMouseEvent('mousedown', rotationHandleClientX, rotationHandleClientY));
      expect(modeContext.getCurrentModeName()).toBe(SelectionModeName.ROTATING);

      modeContext.onMouseMove(createMouseEvent('mousemove', rotationHandleClientX + 20, rotationHandleClientY + 20));
      expect(modeContext.getCurrentModeName()).toBe(SelectionModeName.ROTATING);

      modeContext.onMouseUp(createMouseEvent('mouseup', rotationHandleClientX + 20, rotationHandleClientY + 20));
      expect(modeContext.getCurrentModeName()).toBe(SelectionModeName.SELECTED);
    });
    ...
    it('should transition SELECTED -> ROTATING -> SELECTED, updating region and content on mouse move around center (90deg snaps)', () => {
      ...
      const initialSelectedWorldRegion = { ...selectionSessionManager.getActiveSession()!.getSelectedRegion()! }
      ...
      const mouseDownScreenPos = core.getCamera().worldToScreen(mouseDownWorldHandlePos.x, mouseDownWorldHandlePos.y);
      modeContext.onMouseDown(createMouseEvent('mousedown', mouseDownScreenPos.x, mouseDownScreenPos.y));

      expect(modeContext.getCurrentModeName()).toBe(SelectionModeName.ROTATING);
      let currentSelection = selectionSessionManager.getActiveSession()?.getSelectedContent();
      expect(currentSelection?.data, "Content should not change on mousedown").toEqual(RABBIT_DRAWING);
      expect(currentSelection?.region, "Region should not change on mousedown").toEqual(initialCellRegion);
      ...
      const radius = Math.max(initialSelectedWorldRegion.width, initialSelectedWorldRegion.height) * 0.6;
      ...
      for (const testCase of rotationTestSequence) {
        ...
        modeContext.onMouseMove(createMouseEvent('mousemove', targetScreenPos.x, targetScreenPos.y));

        expect(modeContext.getCurrentModeName(), `Mode check for target angle ${testCase.targetAngleDegrees}deg`).toBe(SelectionModeName.ROTATING);
    ...
    it('should transition SELECTED -> SELECTING -> IDLE if try to rotate only with one char selected', () => {
      ...
      const selectedContent = selectionSessionManager.getActiveSession()?.getSelectedContent()!.data
      expect(selectedContent).toBe('|')

      const handleOffset = 2;
      const righTopScreenCorner = core.getCamera().worldToScreen(
        selectedWorldRegion.startX + selectedWorldRegion.width + handleOffset,
        selectedWorldRegion.startY,
      );

      modeContext.onMouseDown(createMouseEvent('mousedown', righTopScreenCorner.x, righTopScreenCorner.y));
      expect(modeContext.getCurrentModeName()).toBe(SelectionModeName.SELECTING);
    })
    ...
    it('should transition SELECTED -> MOVING -> SELECTED when DRAGGING and MOVING the selected content', () => {
      ...
      const endDragPos = cellToWorld(moveStartCell.x + moveOffsetCells.x, moveStartCell.y + moveOffsetCells.y);
  ...
  describe("Tool State", () => {
    ...
    beforeEach(() => {
      ...
      const selectedContent = activeLayer.readRegion(initialCellStartX, initialCellStartY, DRAWING_WIDTH_CELLS, DRAWING_HEIGHT_CELLS)
  ...
  describe("history", () => {
    ...
    it('Undo/Redo Rotation', () => {
      ...
      modeContext.onMouseMove(createMouseEvent('mousemove', mouseDownScreenPos.x, mouseDownScreenPos.y, 0));

      modeContext.onMouseDown(createMouseEvent('mousedown', mouseDownScreenPos.x, mouseDownScreenPos.y));
      expect(modeContext.getCurrentModeName(), "Mode after mousedown on handle").toBe(SelectionModeName.ROTATING);

      const screenPosFor90Deg = getScreenPosForAngle(90);
      modeContext.onMouseMove(createMouseEvent('mousemove', screenPosFor90Deg.x, screenPosFor90Deg.y));
