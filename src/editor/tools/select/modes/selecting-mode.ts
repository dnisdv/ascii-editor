...
export class SelectingMode implements ISelectionMode<SelectionModeName.SELECTING> {
  ...
  constructor(private coreApi: CoreApi, private selectionSessionManager: SelectionSessionManager, private selectionRender: SelectionRenderer) {
  ...
  handleMouseUp(event: MouseEvent, context: SelectionModeContext): void {
    ...
    this.selectionSessionManager.executeCommandOnActiveSession(new PopulateRegionCommand(this.coreApi, {
      x: this.startPoint!.x,
      y: this.startPoint!.y,
