...
export type SessionEventType = {
  ...
  'session::region_updated': { session: SingleSelectSession; newRegion: Rectangle | null; oldRegion: Rectangle | null };
  'session::content_updated': { session: SingleSelectSession; newContent: SelectedContentEntity | null; oldContent: SelectedContentEntity | null; newBoundingBox: Rectangle | null; oldBoundingBox: Rectangle | null };
  'session::committed': { session: SingleSelectSession; committedContent: SelectedContentEntity | null; beforeContent: SelectedContentEntity | null; targetLayerId: string | null };
...
export class SingleSelectSession extends EventEmitter<SessionEventType> {
  ...
  public setTargetLayerId(id: string | null): void { this._targetLayerId = id; }

  public getSourceLayerId(): string | null { return this._sourceLayerId; }
  public getTargetLayerId(): string | null { return this._targetLayerId; }
  ...
  public updateSelectedContent(newContent: SelectedContentEntity | null): void {
    const oldContent = this.selectedContent ? { ...this.selectedContent, region: { ...this.selectedContent.region } } : null;
  ...
  public static fromSnapshot(session: SingleSelectSession, snapshot: SingleSessionSnapshot): SingleSelectSession {
    ...
    if (snapshot.selectedContent) {
      session.selectedContent = { ...snapshot.selectedContent, region: { ...snapshot.selectedContent.region } };
