...
export abstract class BaseTool<Api extends object = object> implements ITool {
  ...
  constructor(
    { name, bus, requirements, isVisible, hotkey, config, coreApi }: IToolConfig & { requirements?: BaseTool['requirements'] }
  ) {
  ...
  saveConfig(config: Record<string, unknown>) {
    this.bus.tools.emit(
      'tool::update_config::request',
      { name: this.name, config: { ...this.config, ...config } }
    )
  }

  getApi(): Api { return {} as Api }
}
